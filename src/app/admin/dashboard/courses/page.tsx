...
import CourseRow from '@/components/dashboard/courses/CourseRow';
import Filters from '@/components/dashboard/courses/Filters';
import StatsCard from '@/components/dashboard/courses/StatsCard';
...
const CoursesManagementPage: React.FC = () => {
  ...
  const loadCourses = async () => {
    try {
      ...
      updateStats(allCourses); // Update stats with the fetched courses
    } catch (error) {
  ...
  const updateStats = (coursesData: CourseCatalog[]) => {
    const totalCourses = coursesData.length;
    const totalDuration = coursesData.reduce((sum, course) => sum + course.duration, 0);
    ...
    const totalQuizzes = coursesData.reduce((sum, course) => sum + course.totalQuizzes, 0);
  ...
  const handleDeleteCourse = async (courseSlug: string) => {
    ...
    try {
      ...
      const updatedCourses = courses.filter(c => c.slug !== courseSlug);
      setCourses(updatedCourses);
      updateStats(updatedCourses);
    } catch (error) {
