...
import Logo from '@/app/assets/icons/logo-white-full.png'
import GoogleIcon from '@/app/assets/icons/google.svg'
import { useRouter } from "next/navigation";

import { useAuth } from "@/context/AuthContext";
import { SignUpInputs, LoginInputs } from "@/types/auth";
...
export default function AuthPage() {
  ...
  const handleRegister = async (data: SignUpInputs) => {
    ...
    try {
      // Destructure the data to match the auth context's register method signature
      const { email, password, firstName, lastName, role } = data;
      await authRegister(email, password, firstName, lastName, role);
      ...
      // Note: AuthContext will update the user state after successful registration
      // You can remove the router.push here since the useEffect above will handle redirection
      // when user state becomes truthy
  ...
  const handleLogin = async (data: LoginInputs) => {
    ...
    try {
      ...
      // AuthContext will update the user state, and useEffect will handle redirection
  ...
  const handleGoogleAuth = () =>{
    console.log("google auth implementation.")
  }
  ...
  return (
    <>
      {/* Notification Popup */}
      {notification && (
        <div className="fixed top-0 left-0 w-full h-full flex items-start justify-center pt-[5rem] md:pt-[10rem] bg-[transparent] border border-gray-300 mb-2 shadow-md shadow-gray-100/50 z-600">
      ...
      <div className="flex w-full min-h-[100vh] overflow-hidden">
        <div className="hidden wallpaper-wrapper md:flex w-full md:w-[50%] bg-blue-900 items-center justify-center">
          <Image src={Logo} alt="wallpaper" width={300} height={300}  />
        </div>
        ...
        <section className="flex w-full md:w-[50%] flex-col items-center justify-center md:py-[1rem] px-4">
          <div className="container mx-auto gap-2 md:gap-2 md:py-[70px] py-4 lg:gap-6 px-4 flex flex-col items-start justify-center bg-white rounded-[1rem] transition">
            <div className="w-full md:max-w-[500px] mx-auto">
              {tab === "signup" ? (
                <form onSubmit={onSignup(handleRegister)} className="space-y-4 relative">
                  {/* Name Fields */}
                  ...
                  <button 
                    ...
                    className="w-full py-1 md:py-3 font-semibold max-w-[5rem] h-[5rem] mx-auto bg-transparent text-gray-500 flex items-center justify-center gap-2 rounded-lg"
                  >
                    <Image src={GoogleIcon} alt="google logo" width={40} height={40} className="w-full" />
                ...
                <form onSubmit={onLogin(handleLogin)} className="space-y-4 relative">
                  {/* Email */}
                  <div>
                    <input
                      {...log("email", {
